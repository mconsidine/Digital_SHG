...
def solex_proc(serfile, options):
    ...
    Disk, ih, iw, FrameCount, infiledatatype, scalemax = read_video_improved(serfile, fit, LineRecal, options)
    ...
    options["scalemax"] = scalemax #MattC add scalemax to options
    if not 'ratio_fixe' in options and not 'slant_fix' in options:
        ...
        frame, cercle = correct_image(frame / scale_max, phi, ratio), (-1, -1, -1) # Note that we assume 16-bit #MattC
    ...
    return frame, hdr, cercle, scalemax

...
def do_work():
    ...
    for serfile in serfiles:
        ...
        frame, header, cercle, scalemax =sol.solex_proc(serfile,options) #MattC
        print('circle = ' , cercle)
        ...
        frame1[frame1>Seuil_haut]=scalemax
        print('Seuil bas       :', np.floor(Seuil_bas))
        print('Seuil haut      :', np.floor(Seuil_haut))
        fc=(frame1-Seuil_bas)* (scalemax/(Seuil_haut-Seuil_bas))
        fc[fc<0]=0
        frame_contrasted=np.array(fc, dtype=outfiledatatype) #MattC
        ...
        frame1[frame1>Seuil_haut]=scalemax
        fc2=(frame1-Seuil_bas)* (scalemax/(Seuil_haut-Seuil_bas))
        fc2[fc2<0]=0
        frame_contrasted2=np.array(fc2, dtype=outfiledatatype) #MattC
        '''
        # image seuils protus
        frame1=np.copy(frame)
        Seuil_bas=0
        ...
        fc2=(frame1-Seuil_bas)* (scalemax/(Seuil_haut-Seuil_bas))
        fc2[fc2<0]=0
        frame_contrasted3=np.array(fc2, dtype=outfiledatatype) #MattC
        ...
        cc=(cl1-Seuil_bas)*(scalemax/(Seuil_haut-Seuil_bas))
        cc[cc<0]=0
        cc=np.array(cc, dtype=outfiledatatype) #MattC
        '''
        
        # image seuils protus  MattC version
        ...
        frame2=np.array(cl1, dtype=outfiledatatype)
        # sauvegarde le fits
        if options['save_fit']:
